...
const settingsSchema = z
  .object({
    ...
    vipFeeDiscountPercent: z.number().int().min(0).max(100),
    vipEarlyAccessMinutes: z.number().int().min(0).max(10080),
...
export async function PATCH(request: Request) {
  ...
  const settings = await db.platformSettings.update({
    ...
    data: {
      ...
      vipFeeDiscountPercent: parsed.data.vipFeeDiscountPercent,
      vipEarlyAccessMinutes: parsed.data.vipEarlyAccessMinutes,
